...
import utils.TimeManager; // Χρειαζόμαστε τον TimeManager για την ημερομηνία
...
public class PaymentsPanel extends JPanel implements Refreshable {
    ...
    public PaymentsPanel(model.User user) {
    ...
    private class NewPaymentPanel extends JPanel {
        ...
        private void onPayClicked() {
            ...
            try {
                ...
                //1. Εύρεση Λογαριασμού Οφειλής (Bill)
                //Χρησιμοποιούμε τη μέθοδο getBillByRf tou DataStore
                //Αυτό διασφαλίζει ότι ο λογαριασμός ΥΠΑΡΧΕΙ και είναι ΑΠΛΗΡΩΤΟΣ (pending)
                Bill foundBill = BankDataStore.getInstance().getBillByRf(rfInput);

                //CHECK 1: ΕΓΚΥΡΟΤΗΤΑ RF kai ΑΝ ΕΧΕΙ ΕΞΟΦΛΗΘΕΙ 
                if (foundBill == null) {
                ...
                //CHECK 3: ΑΚΡΙΒΕΙΑ ΠΟΣΟΥ
                //Στις RF πληρωμές συνήθως το ποσό είναι συγκεκριμένο
    ...
    private class PendingBillsPanel extends JPanel {
        ...
        public void refresh() {
            ...
            //Παίρνουμε ΟΛΕΣ τις οφειλές από το σύστημα
            List<Bill> allBills = BankDataStore.getInstance().getPendingBills();
            
            for(Bill b : allBills) {
                //Φιλτράρισμα: Δείχνουμε μόνο τους λογαριασμούς που αφορούν το ΑΦΜ του χρήστη
        ...
        private void onPaySelected() {
            ...
            if(selectedBill != null) {
                ...
                //CHECK: ΗΜΕΡΟΜΗΝΙΑ ΕΚΔΟΣΗΣ
                //Ακόμα και αν είναι στη λίστα, ελέγχουμε την ημερομηνία για ασφάλεια
                LocalDate today = TimeManager.getInstance().getDate();
    ...
    // SHARED CONFIRMATION DIALOG kai EXECUTION
    private void showConfirmation(String srcIban, String providerName, String desc, double amount, Bill bill) {
        ...
        addRow(p, g, 4, "Προμήθεια:", "0.00 €"); //Υποθέτουμε δωρεάν πληρωμές προς το παρόν
        ...
        ok.addActionListener(e -> {
            ...
            //1.ΕΚΤΕΛΕΣΗ ΤΗΣ ΠΛΗΡΩΜΗΣ (Μέσω του Command)
            //Το Command θα κάνει withdraw και addTransaction
            new PaymentCommand(srcIban, providerName, amount, desc, bill).execute();
            
            //2.ΔΙΑΓΡΑΦΗ ΤΟΥ BILL ΑΠΟ ΤΙΣ ΟΦΕΙΛΕΣ (CRITICAL STEP)
            //Αυτό εξασφαλίζει ότι το RF "καίγεται" και δεν μπορεί να ξαναπληρωθεί
            if (bill != null) {
                BankDataStore.getInstance().getPendingBills().remove(bill);
                BankDataStore.getInstance().saveAllData(); //Αποθήκευση αλλαγών
            }
            ...
            //Reset Fields (για το NewPaymentPanel)
