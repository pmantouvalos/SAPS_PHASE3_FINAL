...
import service.bridge.InternalProtocol; // <-- Import για το Bridge
...
import service.strategy.InternalFeeStrategy; // Import Strategy
...
import java.util.List; // <-- Βεβαιώσου ότι υπάρχει αυτό το import
...
public class AdminAccountsPanel extends JPanel implements Refreshable {
    ...
    private void createNewAccount() {
        ...
        String[] types = {"Τρεχούμενος", "Ταμιευτηρίου", "Επιχειρηματικός"};
    ...
    private void doCashierTx(Account acc, JTextField f, boolean isDep) {
        try {
            ...
            // Αλλαγή: Χρήση deposit/withdraw αντί για setBalance
            if (isDep) acc.deposit(amt); else acc.withdraw(amt);
            
            // Χρήση FACTORY για Transaction
            TransactionType type = isDep ? TransactionType.DEPOSIT : TransactionType.WITHDRAWAL;
    ...
    private JPanel createTransfersPanel(Account acc) {
        ...
        trP.setBorder(BorderFactory.createTitledBorder("Μεταφορά"));
        ...
        sendBtn.addActionListener(e -> {
            try {
                ...
                // ΔΙΟΡΘΩΣΗ: Πέρασμα του InternalProtocol και του flag false (simulation)
                new TransferCommand(
                        acc.getIban(), 
                        trgIban.getText(), 
                        amt, 
                        new InternalFeeStrategy(), 
                        new InternalProtocol(), // <-- BRIDGE
                        "Admin Transfer",
                        false
                ).execute();
        ...
        payP.setBorder(BorderFactory.createTitledBorder("Πληρωμή (RF)"));
    ...
    private JPanel createStandingOrdersPanel(Account acc) {
        ...
        Runnable load = () -> {
            ...
            // ΔΙΟΡΘΩΣΗ: Χρήση του πλήρους ονόματος για να αποφευχθεί σύγκρουση με το εσωτερικό ListPanel (αν υπήρχε) ή απλά για σαφήνεια.
            // Ωστόσο, το βασικό είναι ότι πρέπει να έχουμε import java.util.List
